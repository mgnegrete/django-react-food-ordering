...
Food ordering web app for **Nana's Pupuseria** — a pop-up pupusa kitchen. Django REST API backend + React/Vite frontend. The project is in early development: the backend has no Django apps yet (only the base project), and the frontend UI has a few landing-page components but no ordering flow.
